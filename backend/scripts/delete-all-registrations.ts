...
// (submission, evaluation, evaluation_score, submission_file). Wipes the
...
async function main() {
  ...
  const [subs] = await pool.query(
    `SELECT TS_ID, hackathon_ID FROM submission WHERE PM_ID = ?`,
    [PM_ID],
  );
  ...
  if (tsIds.length > 0) {
    ...
    await pool.execute(`DELETE FROM submission WHERE TS_ID IN (${ph})`, tsIds);
  }
